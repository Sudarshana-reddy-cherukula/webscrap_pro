...
function ScrapingWorkflow() {
  return (
    <section id="workflow" className="relative border-t border-app-line py-24">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_right,rgba(200,90,72,0.04),transparent_50%)]" />
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-16 grid gap-6 md:grid-cols-4 relative">
          <div className="hidden md:block absolute top-12 left-[calc(12.5%+1.5rem)] right-[calc(12.5%+1.5rem)] h-px bg-gradient-to-r from-amber-500/30 via-orange-500/30 to-rose-500/30" />
          <div className="hidden md:block absolute top-[72px] left-[calc(12.5%+2.5rem)] right-[calc(12.5%+2.5rem)] h-[2px] bg-gradient-to-r from-transparent via-amber-400/20 to-transparent animate-pulse" />
          {steps.map((step, i) => (
            <motion.div
              key={step.title}
            ...
            >
              <div className="absolute -top-1 left-1/2 md:left-8 w-2 h-2 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(200,90,72,0.4)] group-hover:shadow-[0_0_12px_rgba(200,90,72,0.6)] transition-shadow duration-300" />
              <motion.div
              ...
              <h3 className="text-lg font-semibold text-app-fg group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-amber-600 group-hover:to-orange-500 group-hover:bg-clip-text transition-all duration-300">{step.title}</h3>
        ...
        <div className="mt-24 grid gap-12 md:grid-cols-2 items-center">
          ...
          >
            ...
            <pre className="text-xs text-app-muted font-mono leading-relaxed">
              ...
              {'\n'}  <span className="text-amber-700">"url"</span>: <span className="text-emerald-600">"https://example.com/products"</span>,
              {'\n'}  <span className="text-amber-700">"selectors"</span>: <span className="text-app-muted">{'{'}</span>
              {'\n'}    <span className="text-amber-700">"title"</span>: <span className="text-emerald-600">"h2.product-title"</span>,
              {'\n'}    <span className="text-amber-700">"price"</span>: <span className="text-emerald-600">".price-tag"</span>,
              {'\n'}    <span className="text-amber-700">"image"</span>: <span className="text-emerald-600">"img.product-img @src"</span>,
              {'\n'}  <span className="text-app-muted">{'}'}</span>,
              {'\n'}  <span className="text-amber-700">"pagination"</span>: <span className="text-emerald-600">".next-page @href"</span>,
              {'\n'}  <span className="text-amber-700">"rate_limit"</span>: <span className="text-blue-600">2</span>,
              {'\n'}  <span className="text-amber-700">"max_pages"</span>: <span className="text-blue-600">100</span>
              {'\n'}<span className="text-app-muted">{'}'}</span>
            </pre>
          </motion.div>
        </div>
